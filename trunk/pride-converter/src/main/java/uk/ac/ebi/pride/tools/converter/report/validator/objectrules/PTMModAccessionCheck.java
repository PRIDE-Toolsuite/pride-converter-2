 ...
 * @since 0.1
 */
public class PTMModAccessionCheck<ReportObject> extends ObjectRule<ReportObject> {

    private static final Context context = new Context("PTM element");

    public PTMModAccessionCheck(OntologyManager ontologyManager) {
        super(ontologyManager);
        setName("PTMModAccessionCheck");
    }
    ...
    public Collection<ValidatorMessage> check(ReportObject reportObject) throws ValidatorException {
        ...
        if (reportObject instanceof PTM) {
            ...
            if (ptm == null) {
                ...
                if (ptm.getModAccession() == null || "".equals(ptm.getModAccession().trim())) {
                    msgs.add(new ValidatorMessage("PTM modification accession name must not be null: " + ptm.getSearchEnginePTMLabel(),
            ...
            msgs.add(new ValidatorMessage("PTMModAccessionCheck: Could not check the presented object '" + reportObject.getClass()
