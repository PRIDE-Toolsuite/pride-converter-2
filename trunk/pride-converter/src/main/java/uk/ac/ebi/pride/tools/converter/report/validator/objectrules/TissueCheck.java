 ...
 * @since 0.1
 */
public class TissueCheck<ReportObject> extends ObjectRule<ReportObject> {

    private static final Context context = new Context("sampleDescription element");

    public TissueCheck(OntologyManager ontologyManager) {
    ...
    public Collection<ValidatorMessage> check(ReportObject reportObject) throws ValidatorException {
        ...
        if (reportObject instanceof Description) {
            ...
            msgs.add(new ValidatorMessage("TissueCheck: Could not check the presented object '" + reportObject.getClass()
    ...
    private Collection<ValidatorMessage> checkAdmin(Admin admin) {
        if (admin == null) {
            Collection<ValidatorMessage> msgs = new ArrayList<ValidatorMessage>();
            msgs.add(new ValidatorMessage("TissueCheck: Could not check 'admin' section, as the object was null!",
                    MessageLevel.ERROR, context, this));
            return msgs;
        } else {
    ...
    private Collection<ValidatorMessage> checkSampleDescription(Description sampleDesc) {
        ...
        if (sampleDesc == null) {
            ...
            if (!tissueFound) {
                msgs.add(new ValidatorMessage("Could not find a BTO annotation in 'sampleDescription' element!",
                        MessageLevel.ERROR, context, this));
            }
        }
