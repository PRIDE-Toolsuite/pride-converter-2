package uk.ac.ebi.pride.tools.converter.gui.util;

import java.awt.*;

/**
 * Created by IntelliJ IDEA.
 * User: rcote
 * Date: 08/03/12
 * Time: 14:37
 */
public class Colours {

    public static final Color selectedGreen = new Color(153, 255, 153);
    //    public static final Color selectedGreen = new Color(0, 153, 153);
    public static final Color grey = new Color(219, 213, 218);
    public static final Color lightBlue = new Color(153, 153, 255);
    //    public static final Color lightBlue = new Color(100, 117, 245);
    //public static final Color errorRed = new Color(255, 102, 102);
    public static final Color errorRed = new Color(255, 51, 51);
    public static final Color fatalBlack = new Color(51, 51, 51);
    public static final Color warnYellow = new Color(255, 255, 51);
    public static final Color infoBlue = new Color(51, 51, 255);
    public static final Color ambiguousYellow = new Color(255, 255, 153);
}
