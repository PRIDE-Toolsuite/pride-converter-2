...
public class UnimodModification
    ...
    public boolean compareAvg(double mass) {
        return (this.delta.avgeMass.doubleValue() == mass) ? true : false;
    }

    public boolean compareId(int id) {
        return (this.recordId.intValue() == id) ? true : false;
    }

    public boolean isSpecificity(String specificity) {
        for (int i = 0; i < this.specificity.size(); i++) {
            if (this.specificity.get(i).getPosition().compareToIgnoreCase(specificity) == 0) return true;
        }
        return false;
    }
